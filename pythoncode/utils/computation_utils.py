...
def compute_mjp_sde_runs(
	...
	print(f"Starting MJP simulation for {comp_params.run_name} with {comp_params.n_runs_mjp} runs "
		  f"in {len(batches_mjp)} batches.")
	...
	if verbose:
		print(f"Finished MJP simulation for {comp_params.run_name}.")


	batches_sde = get_batchsizes(comp_params.n_runs_sde, comp_params.batchsize_sde)
	paths_batches_sde = []

	print(f"Starting SDE simulation for {comp_params.run_name} with{comp_params.n_runs_sde} runs "
		  f"in {len(batches_sde)}.")
	...
	if verbose:
		print(f"Finished SDE simulation for {comp_params.run_name}.")
